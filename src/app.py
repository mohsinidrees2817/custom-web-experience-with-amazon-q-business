...
if "token" not in st.session_state:
    ...
    if result and "token" in result:
        ...
        client = boto3.client("sso-oidc", region_name=utils.REGION)
        response = client.create_token_with_iam(
            clientId=utils.IDC_APPLICATION_ID,
            grantType="urn:ietf:params:oauth:grant-type:jwt-bearer",
            assertion=st.session_state.token["id_token"],
        )
        
        st.write("Authorization successful", response)
